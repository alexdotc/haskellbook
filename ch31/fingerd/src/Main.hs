...
CREATE TABLE IF NOT EXISTS users
  (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE,
   shell TEXT, homeDirectory TEXT, realName TEXT, phone TEXT)
|]
...
createDatabase = do
  ...
  where meRow :: UserRow
        meRow = 
          (Null, "achavkin", "/bin/zsh",
            "/home/achavkin", "Alex Chavkin", "867-5309")
...
returnUser dbConn soc username = do
  ...
  case maybeUser of
    Nothing -> do
      putStrLn ("Couldn't find matcher user for username: " ++ (show username))
      return ()
